...
import { DxTabPanelModule, DxSelectBoxModule, DxTemplateModule } from 'devextreme-angular';
...
postes: any[] = [];
  ...
  tabsPositions: DxTabPanelTypes.Position[] = ['top'];
  ...
  iconPositions: DxTabPanelTypes.TabsIconPosition[] = ['top', 'start', 'end', 'bottom'];
  iconPosition: DxTabPanelTypes.TabsIconPosition = this.iconPositions[1];
  ...
  constructor(private messageService: MessageService, private  EmoloyeService :EmoloyeService, private router: Router,
     private disciplineService:DisciplineService, private experienceService :ExperienceService, private siteService: SiteService, private posteservice : PosteService , private Directionservice: DirectionService) {}
  ...
  ngOnInit() {
    ...
    console.log('Employé récupéré depuis localStorage:', this.employe);
   
    if (this.employe.photo) {
      this.photoUrl = this.employe.photo; // Assurez-vous que employe.photo contient une URL valide
  }
   // Trouver l'objet correspondant dans `sexes` (si employe.sexe existe)
this.selectedSexe = this.sexes.find(sexe => sexe.name === this.employe.sexe) || null;
    ...
    this.addEmployeeForm = new FormGroup({
      ...
      'dN': new FormControl(this.employe.dateNaissance ||'', Validators.required),
      'dR': new FormControl(this.employe.dateRecrutement ||'', Validators.required),
  ...
  getDisciplines(employeId: number) {
    this.disciplineService.getDisciplinesByEmployeId(employeId).subscribe(data => {
