...
export class PosteComponent {
  ...
  constructor(
   ...
   private employeservice:EmoloyeService,  private cdr: ChangeDetectorRef,
    ...
    if (this.employeId) {
      this.employeservice.getPosteDetails(this.employeId, poste.posteId).subscribe(
        (data) => {
          ...
          console.log('selectedPosteDetails.posteId:', this.selectedPosteDetails.posteId);
        },
        (error) => {
  ...
  deletePoste(poste: any) {
    ...
    if (window.confirm("Voulez-vous vraiment supprimer ce poste ?")) {
      this.employeservice.supprimerPostePourEmploye(this.employeId, posteId).subscribe({
        next: () => {
          // Retirer le poste supprimé du tableau local
          this.postesemploye = this.postesemploye.filter(p => p.posteId !== posteId);
    // Rafraîchir le tableau local (sans reset(), juste en modifiant les données)
  
          // Afficher un message de succès
          this.messageService.add({ severity: 'success', summary: 'Succès', detail: 'Poste supprimé' });
  ...
  savePoste() {
    if (!this.selectedPoste || !this.selectedDirection || !this.selectedSite || !this.poste.dateDebut) {
    ...
    if (this.poste.dateFin) {
      ...
      if (selectedDateFin > today) {
        this.errorMessage = 'Ce poste est en cours. Vous ne pouvez pas spécifier une date de fin avant qu\'il ne soit terminé.';
        return;
      }
    ...
    const dateFin = this.poste.dateFin ? this.formatDate(this.poste.dateFin) : "";
  
    this.employeservice.ajouterPosteAEmploye(
      this.employeId,
  ...
  editPoste(poste: any) {
    ...
    this.employeservice.getPosteDetails(this.employeId, poste.posteId).subscribe({
      next: (response: any) => {
        ...
        this.posteService.getDirectionsByPosteId(poste.posteId).subscribe(directionsData => {
          this.directions = directionsData;
          this.selectedDirection = this.directions.find(d => d.nom_direction === response.nomDirection) || null;
  
          // Charger les sites de la direction sélectionnée
          if (this.selectedDirection) {
            this.directionService.getSitesByDirection(this.selectedDirection.id).subscribe(sitesData => {
              this.sites = sitesData;
              this.selectedSite = this.sites.find(s => s.nom_site === response.nomSite) || null;
            });
          }
        });
  ...
  modifierPoste(poste: any) {
    const posteId = this.selectedPoste?.posteId || this.selectedPosteDetails?.posteId;
    ...
    const dateDebut = this.selectedPosteDetails.dateDebut instanceof Date
      ? this.selectedPosteDetails.dateDebut
      : new Date(this.selectedPosteDetails.dateDebut);
  
    const selectedDateFin = this.selectedPosteDetails.dateFin ? new Date(this.selectedPosteDetails.dateFin) : null;
    ...
    if (selectedDateFin) {
      ...
      if (selectedDateFin > today) {
        this.errorMessageUpdate = 'Ce poste est en cours. Vous ne pouvez pas spécifier une date de fin avant qu\'il ne soit terminé.';
        return;
      }
    ...
    const formattedDateFin = selectedDateFin ? this.formatDate(selectedDateFin) : '';
  
    // Appel à l'API pour modifier le poste
    this.employeservice.modifierPosteAEmploye(
      this.employeId,
      posteId,
      this.selectedDirection.id,
      this.selectedSite.id,
