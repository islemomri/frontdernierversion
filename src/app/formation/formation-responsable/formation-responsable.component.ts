...
verifierDateRappel(formation: any) {
  if (formation.dateRappel && formation.dateRappel > new Date(formation.date_fin_prevue)) {
...
private transformToFormationDto(data: any): FormationDto {
  return {
    ...
    sousTypeFormation: data.sous_type_formation,
    dateDebutPrevue: this.formatDateForCalendar(data.date_debut_prevue),
    dateFinPrevue: this.formatDateForCalendar(data.date_fin_prevue),
                                                 
    responsableEvaluationId: data.responsable_evaluation_id,
    responsableEvaluationExterne: data.responsable_evaluation_externe,
    employeIds: data.employeIds || [],
    responsableEvaluation: data.responsable_evaluation,
    employes: data.employes,
    fichierPdf: data.fichier_pdf,
    organisateurId: data.organisateur_id,
    titrePoste: data.titre_poste,
    valide: data.valide
  };
}
...
setDefaultRappelDate(formation: any) {
  if (formation.date_fin_prevue) {
    const dateFin = new Date(formation.date_fin_prevue);
    dateFin.setDate(dateFin.getDate() - 2); // 2 jours avant la fin
    formation.date_rappel_default = dateFin.toISOString().split('T')[0];
  }
}
loadingDates: { [key: number]: boolean } = {};

onDateSelect(formation: any) {
  if (formation.date_rappel) {
    this.loadingDates[formation.id] = true;
    
    this.formationservice.modifierDateRappel(formation.id, formation.date_rappel)
      .subscribe({
        next: () => {
...
resetToDefault(formation: any) {
  if (formation.date_rappel_default) {
    formation.date_rappel = formation.date_rappel_default;
...
//signatuuuuuuuuuuuuuuuurrrrrrrrrrrrrreeeeeeeeeeeeeee

  // Activer le canvas de signature
  activateSignature() {
    ...
    setTimeout(() => {
      ...
      if (canvas) {
        this.signaturePad = new SignaturePad(canvas, {
          ...
          penColor: 'black', // Couleur de la signature
        });
      } else {
